...
void calcDist()
{
	for (int i = 0; i < n; i++)
	{
		...
		for (int j = 0; j < n; j++)
		{
			if (j != i)
			{
				...
				//if (i==1) printf("(%d %d) (%d %d)= %d %d %d\n", locX[i], locY[i], locX[j], locY[j], dist, dx, dy);
				...
				//if (i==1) printf("%d %d %d %d\n", grafo[i][0], grafo[i][1], dist0, dist1);
			}
		}
	}
	...
	/*printf("##\n");
	for (int i = 0; i < n; i++) {
		printf("%d(%d, %d) %d(%d, %d) %d(%d, %d)\n", i, locX[i], locY[i], grafo[i][0], locX[grafo[i][0]], locY[grafo[i][0]], grafo[i][1], locX[grafo[i][1]], locY[grafo[i][1]]);
	}*/
}
