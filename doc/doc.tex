...
\lstinputlisting[language=c, label=dijkstra, caption={Caminho minimo 1 para todos pesos positivos.}]{src/dijkstra.cpp}
\lstinputlisting[language=c, label=setForest, caption={Floresta dijunta de arvores}]{src/dijointSetForest.cpp}
\lstinputlisting[language=c, label=kruskal, caption={Arvore geradora mínima kruskal}]{src/kruskal.cpp}
...
\lstinputlisting[language=c, label=maxflow, caption={calcula fluxo máximo, Ford-Fulkerson}]{src/maxFlow.cpp}
...
\lstinputlisting[language=c, label=kmp, caption={String matching - Algoritmo KMP - O( n + m)}]{src/kmp.cpp}
